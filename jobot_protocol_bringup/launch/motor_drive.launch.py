...
def wait_for_device(context):
    # 等待 /dev/EtherCAT0 出现，最多 10 s
    for _ in range(100):
        if os.path.exists('/dev/EtherCAT0'):
            return
        time.sleep(0.1)
    raise RuntimeError('IgH EtherCAT master device /dev/EtherCAT0 not found!')



def generate_launch_description():
 # 1. 拉起 IgH 主站（systemd 版）
    start_master = ExecuteProcess(
    ...
    # 2. 等设备节点
    wait_master = OpaqueFunction(function=wait_for_device)
    ...
    robot_description_content = Command(
        [
            ...
            PathJoinSubstitution(
                [
                    FindPackageShare("jobot_protocol_bringup"),
                    "description/config",
                    description_file,
                ]
            ),
